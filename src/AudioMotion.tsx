...
import './AudioMotion.css'
import AudioMotionAnalyzer from './libs/audioMotion-analyzer.js'
...
export default class AudioMotion extends React.Component<Props> {
  motion?: any;
  ...
  componentDidMount(): void {
    ...
    console.log(init)
    if (init === 2) {
      let options = this.props.source ? { source: this.props.source, onCanvasDraw: this.drawcallback.bind(this) } : null;
      this.motion = new AudioMotionAnalyzer(null, options)
      this.motion.height = 500;
    }
  }

  drawcallback() {
    let audioMotion = this.motion;
    const features = {
      showLogo: true,
      energyMeter: true,
      songProgress: false
    ...
    if (features.songProgress) {
      ...
      ctx.beginPath();
      ctx.moveTo(0, posY);
      ctx.lineTo(canvas.width * this.props.source.audioEl.currentTime / this.props.source.audioEl.duration, posY);
      ctx.lineCap = 'round';
